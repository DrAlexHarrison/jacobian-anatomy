...
- Reviewer: a separate session (the release gate is Alex's
  separate session; this report informs revisions, it does not open the gate).
- Date: 2026-07-20. Note version reviewed: v0.9 banner, mtime 02:36.
- My scripts: `audit/equivariance_checks.py` (35/35), `audit/wall_identity_check.py`
  (23/23), `audit/note_checks.py` (15/15, new this pass), all exact,
...
4. Independent note-checks (`note_checks.py`): D squarefree over Q
...
- **R1 (required).** The lifting law's **equality** (and hence the exact
  ...
  identically (my P9, `audit/wall_identity_check.py`); (ii) *no* fiber point
...
- **R2 (required).** "**[COMPUTED]** D squarefree (gcd with its gradient
  ...
  to §5. It is true: certified now in `audit/note_checks.py` (N1, N2).
...
1. **R1**, lifting-law uniqueness: add the y ≡ y(r) forcing identity and the
   ...
   `audit/wall_identity_check.py`, check P9), or cite that file where the
   note says "certificates 1–7".
2. **R2**: certify "D squarefree + irreducible over Q" inside `verify/`
   (one-liners; currently only in `audit/note_checks.py` N1/N2).
